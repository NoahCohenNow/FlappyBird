...
export class EventManager {
  ...
  constructor() {
    this.worker = new Worker('evaluate-threshold', async job => {
        if (job.name === 'check-threshold') {
            await this.evaluateThreshold(job.data.aggregateId);
        }
    }, { 
        connection: {
  ...
  async evaluateThreshold(aggregateId: string) {
    ...
    try {
        ...
        while (cumulativeUsd >= THRESHOLD_USD) {
            ...
            // 2. Publish to WebSocket/Redis PubSub
            const eventData = {
        ...
        console.error('Error in evaluateThreshold:', err);
        throw err;
    }
  }
}
